...
def _run_single(objective_function, n_bats, dim, bounds, iterations,
    ...
    for iteration in range(1, iterations + 1):
        ...
        update_behavior(population, iteration=iteration, alpha=alpha, gamma=gamma, mode="individual")
...
def benchmark(objective_function=sphere,
    ...
    for run_idx in range(n_runs):
        ...
        if verbose:
            print(f"Run {run_idx + 1}/{n_runs} - Best Fitness: {best_fitness:.6e}")
    ...
    if verbose:
        print("=" * 60)
        print(f"Benchmark finished over {n_runs} runs")
    ...
    if plot:
        default_plot_fitness(best_history)
